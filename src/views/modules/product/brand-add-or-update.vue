<template>
  <el-dialog :title="!dataForm.brandId ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-form-item label="显示状态" width="180">
        <template>
          <el-switch :active-value="1" :inactive-value="0" v-model="dataForm.showStatus" active-color="#13ce66"
      ...
      return {
        ...
        dataRule: {
          ...
          firstLetter: [{
            validator: (rule, value, callback) => {
              if (value == null || value === '') {
                ...
                callback(new Error('首字母必须是a-z 或者A-Z任意之间的字母'))
              } else {
                (
                  callback()
                )
          ...
          sort: [{
            validator: (rule, value, callback) => {
              if (value == null || value === '') {
                callback(new Error('序号必须填写'))
              } else if (!Number.isInteger(value) || value < 0) {
                callback(new Error('序号必须是整数且必须大于0'))
              } else {
                (
                  callback()
                )
